...
Документація створена за допомогою **Doxygen** і містить повний опис усіх класів, методів, параметрів та прикладів використання.

###  Формат документування

Коментарі в коді використовують теги:
`@brief` `@param` `@return` `@throws` `@example`

## Автоматична генерація через GitHub Actions

Після кожного push у гілку `feature/docs-ci`:

- запускається GitHub Action,

- генерується нова HTML-документація,

- вона автоматично публікується на GitHub Pages.
...
Офіційна сторінка документації доступна за посиланням:
https://fuw4kr.github.io/Lab-SoftwareDevelopmentTools/
